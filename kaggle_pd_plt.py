...
result_manhattan = sorted_manhattan[sorted_manhattan['Construction year'] >= 2020] \
    .sort_values('price', ascending=False) \
    .head(5)
print(result_manhattan, '\n')

sorted_queens = sorted_data[sorted_data['neighbourhood group'] == 'Queens']
result_queens = sorted_queens[sorted_queens['Construction year'] >= 2020] \
    .sort_values('price', ascending=False) \
    .head(5)
print(result_queens, '\n')

sorted_st_island = sorted_data[sorted_data['neighbourhood group'] == 'Staten Island']
result_st_island = sorted_st_island[sorted_st_island['Construction year'] >= 2020] \
    .sort_values('price', ascending=False) \
    .head(5)
print(result_st_island, '\n')

sorted_brooklyn = sorted_data[sorted_data['neighbourhood group'] == 'Brooklyn']
result_brooklyn = sorted_brooklyn[sorted_brooklyn['Construction year'] >= 2020] \
    .sort_values('price', ascending=False) \
    .head(5)
print(result_brooklyn, '\n')

sorted_bronx = sorted_data[sorted_data['neighbourhood group'] == 'Bronx']
result_bronx = sorted_bronx[sorted_bronx['Construction year'] >= 2020] \
    .sort_values('price', ascending=False) \
    .head(5)
print(result_bronx, '\n')
...
price = [int((result_manhattan['price'][result_manhattan.index[0]])
             .removeprefix('$')
             .replace(',', '')),
         int((result_brooklyn['price'][result_brooklyn.index[0]])
             .removeprefix('$')
         ...
         int((result_bronx['price'][result_bronx.index[0]])
             .removeprefix('$')
             .replace(',', ''))]
service_fee = [int((result_manhattan['service fee'][result_manhattan.index[0]])
                   .removeprefix('$')
                   .replace(',', '')),
               int((result_brooklyn['service fee'][result_brooklyn.index[0]])
                   .removeprefix('$')
                   .replace(',', '')),
               int((result_st_island['service fee'][result_st_island.index[0]])
                   .removeprefix('$')
                   .replace(',', '')),
               int((result_queens['service fee'][result_queens.index[0]])
                   .removeprefix('$')
                   .replace(',', '')),
               int((result_bronx['service fee'][result_bronx.index[0]])
                   .removeprefix('$')
                   .replace(',', ''))]
number_of_reviews = [result_manhattan['number of reviews'][result_manhattan.index[0]].__round__(),
...
column_1 = plt.bar(
    val_1, price, color='blue', width=bar_width, edgecolor='black', label='Price $ per sq/m'
)
column_2 = plt.bar(
    val_2, service_fee, color='red', width=bar_width, edgecolor='black', label='Service fee $'
)
column_3 = plt.bar(
    val_3, number_of_reviews, color='yellow', width=bar_width, edgecolor='black', label='Number of reviews'
)

plt.xlabel('Islands of City', fontdict={'size': 15})
plt.xticks([r + bar_width for r in range(len(price))],
           ['Manhattan', 'Brooklyn', 'Staten Island', 'Queens', 'Bronx'])
plt.ylabel('Amount', fontdict={'size': 15})
plt.ylim(-5, 1200)
plt.title('The overview of real estate prices in New York City', fontdict={'size': 20, 'color': 'blue'})
